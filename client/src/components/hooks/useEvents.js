...
export default function useEvents () {
  ...
  const updateEventsAtLocalStorage = (events) => {
    ...
    const newUsersEvents = usersEvents.filter(userEvents => userEvents.userId !== user.data.id);
  ...
  const getCurrentUserEvents = () => {
    ...
    if (eventsAtStorage) {
      ...
      const currentUserEvents = parsedEvents.find(events => events.userId === user.data.id);
  ...
  const addEvent = (eventBody, eventDate, reminderDate) => {
    ...
    const usersEvents = JSON.parse(localStorage.getItem('events'));
    if (usersEvents) {
      const currentUserEvents = usersEvents.find(userEvents => userEvents.userId === user.data.id);
